...
import struct
import time
from .concrete_message_types import MSG_RESET, msg_types_dict
from .concrete_message_types import MSG_EXECUTED, MSG_STOP, MSG_INFO, MSG_DODATA
from .concrete_message_types import MSG_MOTORDATA, MSG_MOTORSTATE
...
class ExtruderClient():
    def __init__(self, host="192.168.125.60", port=50001): #IP IGUS DRYVE
        self.host = host
        self.port = port
    ...
    def send_reset(self, wait_resp=False):
        return self.__send(MSG_RESET, wait_for_response=wait_resp)
    # Get commands
...
if __name__ == "__main__":
    ec = ExtruderClient(host="192.168.125.220", port=50004)
    ec.connect()
    time.sleep(0.5)
